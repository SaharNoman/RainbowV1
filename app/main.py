...
def send_now(store: str = Query(None), month: str = Query(None)):
    """Send both stock and sales alerts together."""
    stock_result = send_low_stock_email(fetch_low_stock(store))
    latest_month = _get_latest_month()
    m = month or latest_month
    sales_result = send_low_sales_email(fetch_low_sales(m, store), m) if m else {"status": "skipped", "message": "No month"}
    total = (stock_result.get("items_reported") or 0) + (sales_result.get("items_reported") or 0)
    ...
    return {
        "status": "sent",
        "message": f"Stock: {stock_result['status']} | Sales ({m}): {sales_result['status']}",
...
def send_sales_alert(store: str = Query(None), month: str = Query(None)):
    ...
    result = send_low_sales_email(items, m)
    print(f"[Sales Alert] {result}")
    return result


def _get_latest_month():
    """Helper to get the latest month from sales table."""
    try:
        conn = get_raw_conn()
        cur  = conn.cursor()
        cur.execute("SELECT DISTINCT month FROM sales")
